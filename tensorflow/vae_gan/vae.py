...
class VAE(Generator):
    ...
    def __init__(self, hidden_size, batch_size, learning_rate):
        ...
        self.train = layers.optimize_loss(loss, get_or_create_global_step(), learning_rate=learning_rate,
                                          optimizer='Adam', update_ops=[])
    ...
    def __get_reconstruction_cost(self, output_tensor, target_tensor, epsilon=1e-8):
        ...
        return tf.reduce_sum(-target_tensor * tf.log(output_tensor + epsilon) - (1.0 - target_tensor) * tf.log(
            1.0 - output_tensor + epsilon))
